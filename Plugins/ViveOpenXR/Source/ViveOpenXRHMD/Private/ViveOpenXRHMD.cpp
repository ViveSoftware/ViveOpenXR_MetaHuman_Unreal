...
#if PLATFORM_ANDROID
	LoaderHandle = FPlatformProcess::GetDllHandle(TEXT("Vlibopenxr_loader.so"));
	if (LoaderHandle == nullptr)
	{
		UE_LOG(LogViveOpenXRHMD, Error, TEXT("Unable to load Vlibopenxr_loader.so."));
		return false;
	}

	UE_LOG(LogViveOpenXRHMD, Log, TEXT("Vlibopenxr_loader.so loaded."));
